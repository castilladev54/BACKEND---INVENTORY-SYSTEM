...
describe('sale.service — fetchSales() y fetchSaleById()', () => {
  ...
  it('fetchSales retorna solo las ventas del usuario, ordenadas por fecha desc', async () => {
    ...
    await createSaleProcess(userId, [{ product_id: product._id.toString(), quantity: 1, unit_price: 10 }], 'Cash');
    await createSaleProcess(userId, [{ product_id: product._id.toString(), quantity: 1, unit_price: 20 }], 'Debit');
...
describe('adjustment.service — createAdjustmentProcess()', () => {
  ...
  it('✅ commitTransaction: actualiza stock del producto y registra el historial', async () => {
    ...
    const adjustment = await createAdjustmentProcess(
      userId, product._id.toString(), 25, 'initial_count', 'Carga inicial'
    );
  ...
  it('✅ commitTransaction: registra ajuste negativo (mermas/daños)', async () => {
    ...
    const adjustment = await createAdjustmentProcess(
      userId, product._id.toString(), 22, 'damaged', 'Rotura de embalaje'
    );
  ...
  it('🔴 abortTransaction: lanza error si new_stock === stock actual', async () => {
    ...
    await expect(
      createAdjustmentProcess(userId, product._id.toString(), 15, 'correction', '')
  ...
  it('🔴 abortTransaction: lanza error si el producto no existe', async () => {
    ...
    await expect(
      createAdjustmentProcess(userId, fakeId, 50, 'correction', '')
    ).rejects.toThrow('no encontrado');
  ...
  it('fetchAdjustments retorna historial ordenado desc con product_id populado', async () => {
    ...
    await createAdjustmentProcess(userId, product._id.toString(), 10, 'initial_count', '');
    await createAdjustmentProcess(userId, product._id.toString(), 20, 'correction', '');
