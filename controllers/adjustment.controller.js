...
export const createAdjustment = async (req, res) => {
  try {
    ...
    await Promise.all([
      bumpCacheVersion('adjustments', req.userId),
      bumpCacheVersion('products',   req.userId),
      invalidateCache(`product:${product_id}:${req.userId}`)
    ]);
...
export const getAdjustments = async (req, res) => {
  try {
    ...
    const version  = await getCacheVersion('adjustments', req.userId);
    const cacheKey = buildPaginatedKey('adjustments', version, page, limit, req.userId);

    const { data, fromCache } = await getOrSetCache(cacheKey, async () => {
      const [adjustments, total] = await Promise.all([
        fetchAdjustments(req.userId, skip, limit),
        fetchAdjustmentsCount(req.userId)
      ]);
